...
int main()
{
    ...
    // vector<vector<double>> grid = {{5, 10, 20}, {40}, { 60}, {  30}};

    // for (int in = 2; in < num_customers.size()-1; ++in)
    // {
    ...
    //             // pop4.sort_by_domination_crowdingdistance();
    //             // int size_pareto_front = pareto_front_in_generation.back().size();
    //             // for(int index = 0; index < size_pareto_front; ++index)
    //             // {
    ...
    freopen("D:\\Project GA new\\data\\Data_200_75_4.txt", "r", stdin);
                cin >> num_Custom;
                Cus.resize(num_Custom + 1);
                Cus[0] = Customer(0, INT_MAX, 0, 0);
                for (int i = 1; i <= num_Custom; i++)
